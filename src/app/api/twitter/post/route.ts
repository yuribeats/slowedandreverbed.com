...
//   3. Polling media status until processing completes
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // 2. Authenticated client (OAuth 1.0a User Context — required for media upload)
    ...
    // 3. Upload (chunked + waits for processing internally)
    const mediaId = await client.v1.uploadMedia(videoBuffer, {
      mimeType: "video/mp4",
      target: "tweet",
    });

    // 4. Compose + post the tweet
    const text = `${artist} / ${title} — made with automash.xyz`;
